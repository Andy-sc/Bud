    ...
    <div className="card p-5 space-y-3">
      <div className="flex items-start justify-between">
        <div>
          ...
          {debt.owed_to && (
            <p className="text-xs text-[var(--text-muted)]">Le debes a {debt.owed_to}</p>
          )}
        </div>
        <span className="text-xs font-medium text-[var(--text-muted)] tabular-nums">
          {pct(debt.pctPaid)} pagado
        </span>
      </div>
      ...
      <div>
        <div className="flex items-baseline justify-between mb-1.5 text-sm">
          <span className="tabular-nums font-medium text-[var(--text-primary)]">
            {money(debt.currentBalance)} restante
          </span>
          <span className="tabular-nums text-[var(--text-muted)]">
            de {money(debt.original_amount)}
          </span>
        </div>
      ...
      <div className="flex items-center justify-between text-xs text-[var(--text-muted)]">
        <span>Pagado: {money(debt.paidSoFar)}</span>
        {debt.interest_rate > 0 && <span>Interés: {debt.interest_rate}%</span>}
      </div>
